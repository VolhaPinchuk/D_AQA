...
class DashDLOnes00102Cest
{
    ...
    public function removetraining(DashAcceptanceTester $I)
    {
		//variables
		$popup = '.toast-message';
		...
		$chart_count = $I->grabTextFrom('(//*[contains(@class, "card__count")])[1]');
		echo('Count in the chart: ' . $chart_count . "\n");
		...
		//check the chart after remove T
		$I->waitForElementClickable('//button[@id="ShowPopupButton"]', 20);
		$I->click('//button[@id="ShowPopupButton"]');
		$I->waitForElementVisible('(//*[contains(@class, "card__count")])[1]', 20);
		$chart_count1 = $I->grabTextFrom('(//*[contains(@class, "card__count")])[1]');
		echo('Count in the chart after delete added ones: ' . $chart_count1 . "\n");
		$I->waitForElementClickable('//*[contains(@class, "modal-mask")]//button[contains(text(), "Close")]', 20);
		$I->click('//*[contains(@class, "modal-mask")]//button[contains(text(), "Close")]');
		
		//assert count Ones in chart
		$I->assertEquals($chart_count, $chart_count1, 'Count of ones in chart is changed.');
