...
class DashDLOnes00121Cest
{
	...
	//swap Show ones
    public function swapshowones(DashAcceptanceTester $I)
    {
		//variables
		...
		//notification
		$I->waitForElementVisible($position1);
		$I->click($position1);
		$I->wait(3);
		if ($I->elementIsHere('//*[contains(@class, "VNotification")]') === true){
			$I->click(Locator::contains('button', 'OK'));
		...
		//add ones 0.5
		$I->waitForElementVisible('(//input[@type="text"])[1]', 20);
		$I->fillField('(//input[@type="text"])[1]', '0.50');
		$I->waitForElementVisible(Locator::contains('button', 'Confirm'));
		$I->click(Locator::contains('button', 'Confirm'));
			...
			if (($str_cont_1 !== false or $str_empty === true) and ($str_cont_11 !== false or $str_empty1 === true)){
				...
				for ($j=1; $j<=$show_cart_number; $j++){
					...
					if ($item_show_here !== false){
						...
						$req_show1 = '(//*[contains(@class, "header__shows__items")]/div/div[2]//*[@class="row__cell"])[10]';
						$I->waitForElementClickable($req_show1, 20);
						$I->click($req_show1);
