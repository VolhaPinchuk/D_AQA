...
class DashDLOnes00071Cest
{
	...
	//change type from CH
    public function changetypech(DashAcceptanceTester $I)
    {
		//variables
		$popup = '.toast-message';
		...
		for($i = 1; $i <= $itNumber; $i++){
			...
			//RETURN TO CH
			$str_contain = strpos($type, 'NH');
			echo('Strpos: ' . $str_contain . "\n");
			//if type is NH then to CH
			if ($str_contain !== false){
				$I->waitForElementClickable($down_arr, 20);
				$I->click($down_arr);
